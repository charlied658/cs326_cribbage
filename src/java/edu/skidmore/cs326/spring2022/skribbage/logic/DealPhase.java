...
public class DealPhase {
	...
	private void discardToCrib(Card[] toDiscardP1, Card[] toDiscardP2) {
		...
		for (int i = 0; i < toDiscardP1.length; i++) {
			...
			Game.getPlayerList().get(0).removeCardFromHand(toDiscardP1[i], Game.getPlayerList().get(0));
		...
		for (int j = 0; j < toDiscardP2.length; j++) {
			...
			Game.getPlayerList().get(1).removeCardFromHand(toDiscardP2[j], Game.getPlayerList().get(1));
	...
	private void removeCardFromHand(Card cardToRemove, Player p) {
		...
		for (int i = 0; i < theHand.size(); i++) {
			...
			// Character.compare(c.getCardIdentifier(), cardToRemove.getCardIdentifier())){
			// theHand.remove(i);
			// return;
			// }
